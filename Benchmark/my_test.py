...
# =========================================
# List of functions for the algorithm
# =========================================
...
def random_search(fun, lbounds, ubounds, budget):
    ...
    return x_min
# ==============================================
# mysolver optimize function fun. which value x optimizes fun? solver is called in a infinite loop
# ==============================================
def gsa_solver(fun, Lbounds, ubounds, budget):
    #print(fun)
    print(budget)
    i=0
    S, weight_matrix = md.face_extract()
    FBest = LBest = best = bestArg = 0.0
    V = np.zeros([N,dim])
    BestChart = np.zeros([50,1])
    #print(S.shape)
    ...
    fitness = md.Evaluate(weight_matrix, S)
    #print(fitness)
    #print(fitness)
    M = md.MassCalc(fitness)
    G = md.Gconstant(0,50)
    a = md.GField(G, M, S, 0, 50, 1)
    #print(a.shape)
    #print(M.shape)
    S, V = md.move(S, a, V)
    M = md.MassCalc(fitness)
    #print(M) - 1
    S = md.Sbound(S, Lbounds, ubounds)
    best = np.max(fitness)
    ...
    displacement = md.calcDisplacement(S,old_S)
    passive_agents,active_agents = md.checkAgents(displacement,threshold)
    fun = md.diffusion(passive_agents,active_agents,fitness)
    old_S = S
    M = md.MassCalc(fitness)
    G = md.Gconstant(i,MaxIt)
    a = md.GField(G, M, S, i, MaxIt, 1)
    S, V = md.move(S, a, V)

    return FBest
    #fun = Evaluate(weight_matrix, S)
    #fitness = Evaluate(weight_matrix, S)
    """
    for i in (1,MaxIt):
        S = md.Sbound(S, Lbounds, ubounds)
    ...
    return FBest
        ###############################
    #leastArg = np.argmin(fun)
    least = np.min(fun)#finding the least on the updated list
...
def coco_optimize(solver, fun, max_evals, max_runs=1e9):
    ...
    for restarts in range(int(max_runs)):
        ...
        if solver.__name__ in ("random_search","gsa_solver"):
...
##############################################################################
            ...
            solver(fun,lbounds,ubounds,budget)
        else:
